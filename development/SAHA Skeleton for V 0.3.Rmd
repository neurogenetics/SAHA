...
To do:
* Fix Create_SAHA_object to allow for ann to be ran in second modality without overwriting the first
* Ensure Tune_Markers works for query and db
...
ann=Create_SAHA_Viz(ann,meta = meta,facet = TRUE)
...
y=Initialize_MarkerFree(x)

test=Downsample(y)

test2=NormalizeDS(test,assay_query = "RNA")

test3=CorrelateDS(test2)

test4=Create_MarkerFree_Viz(test3,facet = TRUE,meta = meta, ABC = TRUE, chemistry = "10Xv3")

Investigate_MarkerFree(ann = test4,query_cluster = 4,db_cell_type = "327.Oligo.NN")
...
SAHA_cluster.ids <- function(ann) {########REMOVE WHEN IT IS DONE############

   #need to account if there are any insignificant cluster annotations..

   # Extract the desired column
   cluster_ids <- ann$master_df[ann$master_df$top_sig == "T", 3]

   # Generate a sequence from 1 to the length of cluster_ids
   sequence <- 1:length(cluster_ids)

   # Combine sequence numbers with cluster_ids
   new_names <- paste(sequence, sep = "")

   # Create a named vector
   output <- setNames(cluster_ids, new_names)
...
#### liams best match
    ...
    best_match.df <- data.frame(row.names = rownames(test4@results$marker_free$corr),
                            best_match = apply(test4@results$marker_free$corr, 1, function(row) names(test4@results$marker_free$corr)[which.max(row)]),
                            correlation = apply(test4@results$marker_free$corr, 1, max))
